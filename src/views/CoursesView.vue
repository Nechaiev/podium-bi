  ...
  <div class="container mx-auto px-4" v-show="!loading">
    ...
    <form class="grid grid-cols-1  lg:grid-cols-3 gap-4 lg:gap-8 mb-10">
      <input
        ...
        class="p-4 border mb-2 lg:mb-4  :placeholder-text-slate-500 min-h-[60px] w-full]" type="search"
        placeholder="Search">
      <select
      ...
      <input
        @change="onSetFilter('per_page', $event.target.value)" type="number" :value="filter.per_page"
        class="p-4 border  mb-2 lg:mb-4 text-slate-500 min-h-[60px]  min-h-[60px] w-full">
    </form>

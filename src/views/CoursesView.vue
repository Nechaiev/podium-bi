<template>
  <div class="container mx-auto px-4" v-show="!loading">
    ...
    <form class="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-8 mb-10">
      <input
        @change="onSetSearch($event.target.value)"
        class="p-4 border mb-2 lg:mb-4 :placeholder-text-slate-500 min-h-[60px] w-full]"
        type="search"
        placeholder="Search"
      />
      <select
      ...
      <input
        @change="onSetFilter('per_page', $event.target.value)"
        type="number"
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
      <div
        ...
        :key="course.id"
        class="flex flex-col p-5 border"
      >
        <img src="/assets/img/courses.webp" class="mb-6" alt="courses" />
        ...
        <div class="flex justify-between mt-auto">
          <router-link
            ...
            class="inline-flex items-center justify-center font-bold border shadow-xl rounded cursor-pointer min-h-[42px] bg-purple-500 text-white hover:bg-purple-700 rounded-none py-2 px-6"
          >
            <b>See more</b>
          </router-link>

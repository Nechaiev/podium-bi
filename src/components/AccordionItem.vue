  ...
  <div class="overflow-hidden rounded-[20px]">
    <slot name="header" :toggle="onToggle" :isExpanded="isExpanded">
      <button @click="onToggle"
              class="relative flex flex-wrap gap-3 justify-between items-center pl-[30px] py-[30px] pr-24 bg-white w-full">
        <span v-if="title" class="text-left text-4xl block">
          {{ title }}
        </span>
        <span
          class="absolute top-[30px] right-[30px] w-10 h-10 flex items-center justify-center bg-bgBody text-primaryColor rounded-full">
